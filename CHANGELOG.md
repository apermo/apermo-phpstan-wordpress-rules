...
- WpQueryNoFoundRowsRule: warn when `new WP_Query()` is missing `no_found_rows => true`
- AutoloadComplexDataRule: warn when `update_option()`/`add_option()` autoloads array-typed data
- SprintfTranslationMismatchRule: flag `sprintf()` placeholder count mismatch vs translation args

## [0.3.0] - 2026-03-07

### Added

- ActionDocParamMismatchRule: detect `@param` count/type mismatches in `do_action()` PHPDoc blocks
- FilterDocParamMismatchRule: detect `@param` count/type mismatches in `apply_filters()` PHPDoc blocks
